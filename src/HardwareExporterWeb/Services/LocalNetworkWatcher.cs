...
public class LocalNetworkWatcher : ILocalNetworkWatcher
{
    ...
    public async Task<IEnumerable<Neighbor>> GetLocalNeighborsAsync()
    {
        ...
        {
            if (!localAddress.ToString().Contains("10.100.100")) continue;
            ...
            var range = new IPAddressRange(new IPAddress(networkAddress.Address + 1), new IPAddress(broadcastAddress.Address - 1));
